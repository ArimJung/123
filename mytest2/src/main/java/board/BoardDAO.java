...
//	final String sql_selectAll_B = "SELECT * FROM (SELECT * FROM BOARD ORDER BY BID DESC) WHERE ROWNUM <=?";
	final String sql_selectAll_B = "SELECT * FROM (SELECT * FROM BOARD1 JOIN MEMBER ON BOARD1.MID = MEMBER.MID"
								  +" ORDER BY BID DESC) WHERE ROWNUM <=?";
	
	final String sql_selectAll_B_M = "SELECT * FROM (SELECT * FROM BOARD1 JOIN MEMBER ON BOARD1.MID = MEMBER.MID"
...
//								    +" WHERE MNAME LIKE '%'||?||'%' ORDER BY BID DESC) WHERE ROWNUM <=?";
	...
	final String sql_selectAll_B_size= "SELECT * FROM BOARD1";
	final String sql_selectAll_B_M_size= "SELECT * FROM BOARD1 JOIN MEMBER ON BOARD1.MID = MEMBER.MID WHERE MEMBER.MNAME=?";

		// LIMIT 은 MySQL
		// Oracle 은 ROWNUM 을 사용함 (※ SQL문 실행순서 유의!)
//	final String sql_selectAll_R="SELECT * FROM REPLY WHERE BID=? ORDER BY RID DESC";
	final String sql_selectAll_R="SELECT * FROM REPLY1 JOIN MEMBER ON REPLY1.MID = MEMBER.MID WHERE BID=? ORDER BY RID DESC";
	final String sql_insert ="INSERT INTO BOARD1(BID,MID,MSG) VALUES((SELECT NVL(MAX(BID),0)+1 FROM BOARD1),?,?)";
	final String sql_insertR="INSERT INTO REPLY1 VALUES((SELECT NVL(MAX(RID),0)+1 FROM REPLY1),?,?,?)";
	final String sql_delete="DELETE FROM BOARD1 WHERE BID=?";
	final String sql_deleteR="DELETE FROM REPLY1 WHERE RID=?";
	final String sql_update="UPDATE BOARD1 SET FAVCNT=FAVCNT+1 WHERE BID=?";
